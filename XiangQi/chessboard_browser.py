...
def print_chessboard(xq_chessboard: AGIObject):
    ...
    for i in xq_chessboard.attributes['xq::pieces'].value:
        name = i.attributes['xq::piece_name'].concept_id[4:]
        if i.attributes['xq::piece_owner'].concept_id == 'xq::red_team':
            name = name.upper()
        else:
            name = name.lower()
        position = [None, None]
        position[0] = to_int(i.attributes['xq::position'].agi_list().get_element(0))
        position[1] = to_int(i.attributes['xq::position'].agi_list().get_element(1))
